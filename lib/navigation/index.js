...
const Navigation = {
  ...
  events: () => ({
    ...
    registerComponentDidDisppearListener: () => {},
    bindComponent: () => {},
  }),
};
...
export const setCredentialDashboardRoot = (showUnlockModal) => {
  Navigation.setRoot(credentialDashboardRoot).then(()=>{
    if (showUnlockModal)
...
// Session start
export const startSession = ({sessionPointer, exitAfter = false}) => {
...
// Session pointer of previously started IRMA session
var prevSessionPointerU = null;

// Session with navigation change
export const startSessionAndNavigate = ({sessionPointer, exitAfter = false, setRoot = false, showUnlockModal = false, componentId = CREDENTIAL_DASHBOARD_ROOT_ID}) => {

  // Check whether session is not the same as the previous session to catch duplicate events
  if (prevSessionPointerU !== sessionPointer.u) {
    const sessionId = startSession({sessionPointer, exitAfter});
  ...
  prevSessionPointerU = sessionPointer.u;
};
...
// // Events
export const componentAppearanceChangedListener = () => {
  Navigation.events().registerComponentDidAppearListener(({ componentId }) => {
    if (componentId === CREDENTIAL_DASHBOARD_ROOT_ID) {
      // Enable the sidebar on the CredentialDashboard
      ...
      if (nbVariables.platform === 'android')
        hideCredentialDashboardSidebar();
    }
  });

  Navigation.events().registerComponentDidDisappearListener(({ componentId }) => {
    // Disable the sidebar outside the CredentialDashboard
    if (componentId === CREDENTIAL_DASHBOARD_ROOT_ID)
