...
export default class RepeatedValueForm extends Component {
    ...
    return (
      <Form style={{paddingRight: 20}}>
        ...
        <FormInput
          ...
          validate={(repeatValue, typeValidate) => typeValidate(repeatValue) && repeatValue === firstValue}
        />
      </Form>
    );
  }
  ...
  renderError() {
    const { validationForced, invalidMessage } = this.props;
    const { firstValue, repeatValue } = this.state;
