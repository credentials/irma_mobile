...
export const t = namespacedTranslation('CollapsableForm');
    ...
    return (
      <View>
        ...
        <View style={{marginVertical: 10, justifyContent: 'center', flexDirection: 'row'}}>
          { !onSkip ? null :
              <Button testID="skipButton" bordered primary onPress={onSkip} style={{marginRight: 20}}>
                <Text>{ t('.skip') }</Text>
              </Button>
          }
          <Button testID="nextButton" {...buttonProps} onPress={onNext}>
            <Text>{ t('.next') }</Text>
          </Button>
        </View>
      </View>
