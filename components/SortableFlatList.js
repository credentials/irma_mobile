  ...
  render() {
    ...
    return (
      <View ref={setRef(index)} collapsable={false} style={{ opacity: 1, flexDirection: horizontal ? 'row' : 'column' }}>
        { !spacerSize ? null : this.renderSpacer(spacerSize) }
        <View style={[
        ...
        { !endPadding ? null : this.renderSpacer(endPadding) }
      </View>
    );
  }
}
...
const styles = StyleSheet.create({
  ...
  wrapper: {
    flex: 1,
    // Setting { opacity: 1 } fixes Android measurement bug: https://github.com/facebook/react-native/issues/18034#issuecomment-368417691
    opacity: 1,
  },
});